...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item active text-capitalize" aria-current="page"> class wise student holiday attendance report</li>
  </ol>
</nav>
            </div>
        </div>
...
<div class="input-group" align="right">
    <a href="holiday-contact-lists?id=<?php echo $instid; ?> && cls=<?php echo $cles; ?> && sectin=<?php echo $sectn; ?> && sesin=<?php echo $sesin; ?> && dtes=<?php echo $dates; ?>" target="_blank" class='btn btn-success text-uppercase'><i class="fa-solid fa-file-excel"></i> Excel</a>
    <a href="holiday-students-record?id=<?php echo $instid; ?> && cls=<?php echo $cles; ?> && sectin=<?php echo $sectn; ?> && sesin=<?php echo $sesin; ?> && dtes=<?php echo $dates; ?>" target="_blank" class='btn btn-danger text-uppercase'>
...
<table class="table table-reponsive w-100" style='background:hsl(35, 100%, 80%);'>
    <thead>
        ...
        <tr align="center">
            <th colspan="9" class="fs-5">Student Holiday Report <?php echo $dates; ?></th>
        </tr>
        <tr class="bg-apna">
...
<?php
$at=1;
$sl_attendance = mysqli_query($con,"select * from attandance where instituteId='$instid' && class='$cles' && section='$sectn' && session='$sesin' && date='$dates' && status='H'");

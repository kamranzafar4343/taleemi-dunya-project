...
<br><br>
    <div class="row">
        <div class="col">
<form method="post" enctype="multipart/form-data">
<table class="table table-responsive w-100 bg-apna-body">
    <tr>
        <td>
            <input type="file" class="form-control" name="studet_img" accept=".jpg, .jpeg, .png">
        </td>
    </tr>
    <tr>
        <td colspan="8">
...
<tr align="center" style="background-color:hsl(30,100%,80%);">
    ...
    <td style="border:1px solid hsl(25, 100%, 50%);" width="50"><img src="student_imgs/<?php echo $image; ?>" class="img-fluid"><input type="hidden" value="<?php echo $image; ?>" name="stimge[]"></td>
...
if(isset($_POST['btn_lect'])){
    ...
    $studet_img = $_FILES['studet_img']['name'];
    $studet_img_tmp = $_FILES['studet_img']['tmp_name'];
    $path = "daily-diary/".$studet_img;
    move_uploaded_file($studet_img_tmp,$path);
    
    $inst_id = $_POST['inst_id'];
    $dil_dat = date("j-m-Y");

foreach ($st_nme as $key => $value) {
  $inst_lectres = mysqli_query($con,"insert into dailydairy (student_name,father_name,class,section,session,roll_number,links,instituteId,deliver_date,files) values ('".$value."','".$fnme[$key]."','".$cles[$key]."','".$sctn[$key]."','".$sesns[$key]."','".$rlnmbr[$key]."','$inpt','$inst_id','$dil_dat','$studet_img')"); }
if($inst_lectres){ echo "<script>swal.fire('Good Jod!', 'Diary Successfully Send', 'success');</script>"; }else{ echo "<script>swal.fire('Oh Sorry!', 'Diary is not Send', 'error');</script>"; }   
}
?>
        </div>
    </div>    

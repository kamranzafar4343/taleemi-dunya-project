...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item active text-capitalize" aria-current="page"> teacher gallery</li>
  </ol>
</nav>                
            </div>
        </div>
...
if(isset($_POST['btn_grly'])){
    ...
    $path = "staff-gallery/".$uimgs;

if(!in_array($ext,$allow)){
echo "<script>swal.fire('Sorry!','Only Allowed the PNG, JPG and JPEG Files.','info');</script>";
}else{
move_uploaded_file($uimgs_tmp,$path);
$inst_students = mysqli_query($con,"insert into teacherGallery (instituteId,files) values ('$instiid','$uimgs')");
...
<?php
$sl_img = mysqli_query($con,"select * from teacherGallery where instituteId='$userId'");
if(mysqli_num_rows($sl_img)>0){
...
?>
        <div class="col-6 col-sm-4 col-md-3 col-lg-2 col-xl-2 col-xl-2 mb-3">
            <div class="card bg-apna">
                <img src="staff-gallery/<?php echo $files; ?>" class="card-img-top">
                <div class="card-body" style="border:1px solid hsl(25, 100%, 50%);background:hsl(35, 100%, 80%);">
<div class="d-flex">
    <div class="flex-fill p-2 text-center">
        <a href="#"><i class="fa fa-edit text-success"></i></a>
    </div>
    <div class="flex-fill p-2 text-center">
        <a href="delete-teacher-gallery?id=<?php echo $id; ?>"><i class="fa fa-trash-alt text-danger"></i></a>
    </div>
</div>
                </div>
            </div>

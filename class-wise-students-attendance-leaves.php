...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item active text-capitalize" aria-current="page"> class wise student leave attendance report</li>
  </ol>
</nav>
            </div>
        </div>
...
<div class="input-group" align="right">
    <a href="leaves-list-contact?id=<?php echo $instid; ?> && cls=<?php echo $cles; ?> && sectin=<?php echo $sectn; ?> && sesin=<?php echo $sesin; ?> && dtes=<?php echo $dates; ?>" target="_blank" class='btn btn-success text-uppercase'><i class="fa-solid fa-file-excel"></i> Excel</a>
    <a href="leaved-students-record?id=<?php echo $instid; ?> && cls=<?php echo $cles; ?> && sectin=<?php echo $sectn; ?> && sesin=<?php echo $sesin; ?> && dtes=<?php echo $dates; ?>" target="_blank" class='btn btn-danger text-uppercase'>
...
<table class="table table-reponsive w-100" style='background:hsl(35, 100%, 80%);'>
    <thead>
        ...
        <tr align="center">
            <th colspan="10" class="fs-5">Student Leave Report <?php echo $dates; ?></th>
        </tr>
        <tr>
...
<?php
$at=1;
$sl_attendance = mysqli_query($con,"select * from attandance where instituteId='$instid' && class='$cles' && section='$sectn' && session='$sesin' && date='$dates' && status='L'");

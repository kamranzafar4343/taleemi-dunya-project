...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item active text-capitalize" aria-current="page"> Student Alerts</li>
  </ol>
</nav>                
            </div>
        </div>
    ...
    <tr>
        ...
        <td colspan="3">
            <button class="btn btn-success text-uppercase" type="submit" name="btn_lect">
                <i class="fa-regular fa-paper-plane"></i> send alerts to selected student
            </button>
        </td>
    </tr>
...
foreach ($st_nme as $key => $value) {
  $inst_lectres = mysqli_query($con,"insert into studentAlerts (student_name,father_name,class,section,session,roll_number,links,instituteId,deliver_date) values ('".$value."','".$fnme[$key]."','".$cles[$key]."','".$sctn[$key]."','".$sesns[$key]."','".$rlnmbr[$key]."','$inpt','$inst_id','$dil_dat')"); }

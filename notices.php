...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item active text-capitalize" aria-current="page"> student notices</li>
  </ol>
</nav>                
            </div>
        </div>
    ...
    <form method="post" enctype="multipart/form-data">
        <div class="row">
            <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 col-xxl-12 mb-3">
                ...
                <label class="text-capitalize">message for all students</label>
...
$inst_notc = mysqli_query($con,"insert into studentNotices (instituteId,messages) values ('$instid','$notce')");
if($inst_notc){ echo "<script>swal.fire('Good Job!','Notice Issue Successfully All Staff.','success');</script>"; }else{ echo "<script>swal.fire('Oops!','Notice is not Issue.','error');</script>"; }    
}
?>    
</div>
...
<h4 class="fs-4 fw-bold text-uppercase border-bottom border-white text-white">notice manager</h4>
        </div>
<?php
$sl_ntic = mysqli_query($con,"select * from studentNotices where instituteId='$userId'");
...
?>
        <div class="col-12 col-sm-6 col-md-6 col-lg-6 col-xl-6 col-xl-6 mb-3">
            <a href="delete-student-notices?id=<?php echo $id; ?>" class="nav-link">
<div class="card p-2 text-capitalize" style='background:hsl(35, 100%, 70%);'>
    <?php echo $messages; ?>
</div>
            </a>
        </div>

...
foreach ($st_nme as $key => $value) {
  $inst_lectres = mysqli_query($con,"insert into dailydairy (student_name,father_name,class,section,session,roll_number,links,instituteId,deliver_date) values ('".$value."','".$fnme[$key]."','".$cles[$key]."','".$sctn[$key]."','".$sesns[$key]."','".$rlnmbr[$key]."','$inpt','$inst_id','$dil_dat')"); }
if($inst_lectres){
    echo '<script>

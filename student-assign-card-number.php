...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item"><a class="text-white text-capitalize text-decoration-none" href="javascript:void()" onclick="location.href='student-asign-card'"> student asign card</a></li>
...
if(isset($_GET['id'])){
    $ids = $_GET['id'];
    $sel_crd = mysqli_query($con,"select * from addStudents where id='$ids' && instituteId='$userId'");
...
$duplicate = mysqli_query($con,"select * from addStudents where digitalCard='$crd'");
if(mysqli_num_rows($duplicate)>0){
    echo "<script>swal.fire('Carefully Read!','This Card Already Assigned.','info');</script>";
}else{
$sl_crd = mysqli_query($con,"update addStudents set digitalCard='$crd' where id='$ids'");

...
if(mysqli_num_rows($duplicate)>0){
    ...
    foreach ($st_sbjct as $key => $value) {
  $inst_rstls = mysqli_query($con,"insert into results(student_name,father_name,student_img,class,section,session,attendance,roll_no,subject,total_marks,marks,passing_marks,terms,months,remarks,instituteId,date,resultId)values('$st_name','$ft_name','$st_image','$st_cls','$st_sectn','$st_sesin','$st_atednce[$key]','$st_rolnum','$st_sbjct[$key]','$totl_mrks[$key]','$st_mrks[$key]','$pas_mrks[$key]','$trm_id','$mnt','Enter Exame Remarks','$institu_id','$dt','$rsltid')");    
  }

...
$td = 1;
$selct_othr_list = mysqli_query($con,"select * from otherChargesPaid where months='$chrge_months' && session='$chrge_sesion' && instituteId='$institut_ids'");
...
<table class="w-100">
    <thead>
        ...
        <tr style="background-color:hsl(35,100%,80%);">
<th colspan="9"><div class="text-capitalize text-center">monthly other charges month of "<?php echo $monthNames; ?>" & Session <?php echo $chrge_sesion; ?></div></th>
        </tr>
        <tr class="bg-apna" align="center">
            ...
            <th style="border:1px solid black;">Charges Name</th>
...
<tr style="background-color:hsl(35,100%,80%);">
    ...
    <td style="border:1px solid black;"><?php echo $chrge_date; ?></td>
...
<?php }
$sel_othr_list = mysqli_query($con,"select SUM(receive_amount) as othrschrgs otherChargesPaid where months='$chrge_months' && session='$chrge_sesion' && instituteId='$institut_ids'");
while($chrgs = mysqli_fetch_array($sel_othr_list)){
    $othrschrgs = $chrgs['othrschrgs'];
}
?>
<tr style="background-color:hsl(35,100%,80%);">

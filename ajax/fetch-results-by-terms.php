...
$schol = mysqli_fetch_assoc($selct_schol);
$institute_name = $schol['institute_name'];
$campus = $schol['campus'];
$logo = $schol['logo'];
$ranks = 1;
$sel_rst = mysqli_query($con,"select * from finalResults where instituteId='$inst_ids' && class='$aply_cls' && section='$aply_sectn' && term='$aply_trms' && session='$aply_sesin' order by overall_obtained desc");
while($ranks <= 10000 && $result = mysqli_fetch_array($sel_rst)){
$ids = $result['id'];
...
$sl_trms = mysqli_query($con,"select * from addTerms where instituteId='$inst_ids' && termid='$term'");
$tm = mysqli_fetch_assoc($sl_trms);
$terms = $tm['term'];
?>
<div class="col-12 mb-3 " style="padding-right:3rem;height:95rem;overflow:hidden;">
    <table class="table table-responsive table-striped w-100" style="background:hsl(35,100%,80%);">
<tr>
    <th width="50"><img src="portal-admins/institute-logos/<?php echo $logo;?>" class="img-fluid"></th>
    <th colspan="8">
        <h1 class="p-0 fs-1 fw-bolder text-uppercase text-center"><?php echo $institute_name; ?></h1>
...
<tr align="center">
    ...
    <td style="border:1px solid black;"><?php if($rankg === 1){ echo $rankg."st"; }elseif($rankg === 2){ echo $rankg."nd"; }elseif($rankg === 3){ echo $rankg."rd"; }else{ echo $rankg."th"; }  ?></td>
</tr>
<tr>
...
<?php
$attd_total = mysqli_query($con,"select * from results where instituteId='$inst_ids' && terms='$term' && roll_no='$rollnumber'");
...
<?php
$attd_presnt = mysqli_query($con,"select * from results where instituteId='$inst_ids' && terms='$term' && roll_no='$rollnumber' && attendance='P'");
...
<?php
$attd_abst = mysqli_query($con,"select * from results where instituteId='$inst_ids' && terms='$term' && roll_no='$rollnumber' && attendance='A'");

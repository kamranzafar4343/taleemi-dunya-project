...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item active text-capitalize" aria-current="page"> stafff inquiry today report</li>
  </ol>
</nav>                
            </div>
        </div>
...
$newdt = date("j-m-Y");
$sl_stud = mysqli_query($con,"select * from staffInquiry where InstituteID='$userId' && dates='$newdt' order by id desc");
$counts = mysqli_num_rows($sl_stud);
if($counts == 0){ echo "<div class='fs-6 text-capitalize alert alert-danger alert-dismissible fade show'>Today Staff inquiry is empty.
...
$demandSalary = $rows['demandSalary'];
